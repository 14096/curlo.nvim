...
local function resolve_and_run(argv, lines, captures, output_file, jq_filter)
  ...
  runner.run(resolved, config.values, captures, output_file, jq_filter)
end
...
function M.run_curl_at_cursor()
  ...
  local argv, output_file, jq_filter = parser.find_at_cursor(lines, cursor_row)
  if not argv then
  ...
  resolve_and_run(argv, lines, captures, output_file, jq_filter)
end
...
function M.run_all()
  ...
  for i, cmd in ipairs(cmds) do
    ...
    resolve_and_run(cmd.argv, lines, caps, cmd.output_file, cmd.jq_filter)
  end
end
